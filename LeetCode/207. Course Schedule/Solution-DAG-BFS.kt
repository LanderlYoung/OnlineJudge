...
class Solution {
  /**
   * The BFS (or not necessarily do it the BFS way) is inspired by https://www.cs.princeton.edu/~wayne/kleinberg-tardos/pdf/03Graphs.pdf
   * 1. for a dag G, there must at least on node with 0-in degree (proved in the slide)
   * 2. we take that node (say v) out, so G-v is still a dag
   * 3. Do it repeatedly, we would find a order of 0-in degree nodes. which would be a valid topological order.
   *
   * Note:
   * - in step 2. if we took all 0 in-degree node at once, it's a BFS.
   * - however we can take them at any order ;)
   */
  fun canFinish(numCourses: Int, prerequisites: Array<IntArray>): Boolean {
    ...
    val zeroInDegreeNodes = ArrayDeque<Int>()
    inDegree.entries.forEach {
      if(it.value == 0) {
        zeroInDegreeNodes.add(it.key)
      }
    }

    while (zeroInDegreeNodes.isNotEmpty()) {
      val node = zeroInDegreeNodes.removeFirst()
