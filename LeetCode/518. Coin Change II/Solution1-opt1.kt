...
class Solution {
  fun change(amount: Int, coins: IntArray): Int {
    ...
    val dp = Array(coins.size + 1) { arrayOfNulls<Pair<Int, Int>?>(amount + 1) }
    for (i in dp.indices) {
      dp[i][0] = 0 to 1
    }
    for (i in 0..amount) {
    ...
    for (c in 1..coins.size) {
      for (n in 1..amount) {
        ...
        // don't use coin[c - 1]
        val unselectedSum = dp[c - 1][n]!!.run { first + second }
        ...
        while (left >= 0) {
          if (left == 0) {
            ...
            selectedSum += dp[c - 1][left]!!.run { first + second }
          }
          left -= denomination
        }
        dp[c][n] = selectedSum to unselectedSum
      }
    }
    return dp[coins.size][amount]!!.run { first + second }
  }
}
